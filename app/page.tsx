...
import { Save, Copy, Plus, X, ChevronDown, ChevronUp, Clock, Folder, BrainCircuit, MessageSquareText, Zap, CloudUpload, CloudDownload, CheckCircle2, Link2, ListOrdered, Edit3, Trash2, Search } from 'lucide-react';
...
export default function CSVCreator() {
  ...
  // Sync master URLs to local storage
  useEffect(() => {
  ...
  const removeMasterUrl = (id: string) => { 
    ...
    setRows(rows.map(r => { 
      ...
      return { ...r, orderedSelections: filteredSelections, targetUrls: compileTargetUrls(filteredSelections, r.manualUrl) }; 
    })); 
  };
  ...
  const clearAllRows = () => { 
    if(window.confirm("Are you sure you want to completely clear the screen?")) { 
  ...
  const handleManualUrlTextChange = (rowId: string, text: string) => { 
    setRows(rows.map(r => { 
      if (r.id !== rowId) return r; 
      return { ...r, manualUrl: text, targetUrls: compileTargetUrls(r.orderedSelections, text) }; 
    })); 
  };
  ...
  // 🌟 FULLY RESTORED: Real AI Logic Endpoint Execution
    ...
    try {
      ...
      if (resData.success && Array.isArray(resData.data)) {
         const newAiRows = resData.data.map((aiRow: any, index: number) => ({ 
           ...
           manualUrl: '', 
           isManualUrlActive: false, 
           targetServer: aiRow.targetServer || 'None', 
         ...
         alert(`✅ AI Data inserted at Row #${insertPosition + 1}!`);
      } else {
  ...
  // Fetch profiles list for Cloud Browser panel
  const fetchCloudProfiles = async () => {
    setIsSyncing(true);
    try {
      const res = await fetch('/api/cloud', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ action: 'list' }) });
      const resData = await res.json();
      if (resData.success) {
        setCloudProfiles(resData.data);
        setShowCloudBrowser(true);
      } else { alert(`❌ Fetch Error: ${resData.error}`); }
    } catch (err: any) { alert(`❌ Network Error: ${err.message}`); } finally { setIsSyncing(false); }
  };

  const loadProfileFromBrowser = (keyToLoad: string) => {
    setSyncKey(keyToLoad);
    syncToCloud('load', keyToLoad);
    setShowCloudBrowser(false);
  };

  const syncToCloud = async (action: 'save' | 'load' | 'delete', overrideKey?: string) => {
    const targetKey = overrideKey || syncKey;
    if (!targetKey) return alert("Enter Sync Key!");
    if(action === 'delete' && !overrideKey) { const confirmDelete = window.confirm(`Are you sure you want to PERMANENTLY DELETE all cloud data for this Key?`); if(!confirmDelete) return; }
    
    setIsSyncing(true);
    try {
      const res = await fetch('/api/cloud', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ action, syncKey: targetKey, data: { rows, masterUrls } }) });
      const resData = await res.json();
      if (resData.success) {
        if (action === 'load') {
          if (resData.data && resData.data.rows) { 
            setRows(resData.data.rows); 
            setMasterUrls(resData.data.masterUrls || []); 
            alert("✅ Sync Success!"); 
          } else { 
            alert("⚠️ No data found for this Sync Key on the cloud."); 
          }
        } else if (action === 'save') { 
          alert("✅ Vercel Cloud backup locked!"); 
        } else if (action === 'delete') { 
          alert("🗑️ Cloud Data Deleted!"); 
          if(showCloudBrowser) fetchCloudProfiles(); 
        }
  ...
  const openCalendar = (e: React.MouseEvent<HTMLInputElement>) => {
    try { if (typeof e.currentTarget.showPicker === 'function') { e.currentTarget.showPicker(); } } catch (err) {}
  };
  ...
  return (
    <div className="min-h-screen bg-[#0A0A0C] text-gray-200 p-6 font-sans">
      ...
      {/* HEADER SECTION */}
      <div className="flex flex-col xl:flex-row justify-between items-start xl:items-center mb-6 border-b border-gray-800 pb-4 gap-4">
        <div>
        ...
        <div className="flex items-center gap-2 bg-[#12121A] p-2 rounded-lg border border-gray-800 shadow-md flex-wrap">
          <input type="password" placeholder="Hidden Key ••••" className="bg-[#0A0A0C] border border-gray-700 rounded-md p-2 text-white outline-none focus:border-[#5842ff] w-32 text-sm font-mono tracking-widest" value={syncKey} onChange={(e) => setSyncKey(e.target.value)} />
          ...
          <button onClick={fetchCloudProfiles} disabled={isSyncing} className="flex items-center gap-1 px-4 py-2 bg-[#5842ff] text-white rounded hover:bg-[#4635d8] transition text-sm font-bold shadow-lg"><Search size={16} /> Browse Cloud</button>
        </div>
      </div>

      {/* CLOUD BROWSER SHIELDED LIST */}
      {showCloudBrowser && (
        <div className="bg-[#12121A] p-5 rounded-xl border border-[#5842ff]/50 mb-6 shadow-[0_0_15px_rgba(88,66,255,0.15)] transition-all">
          <div className="flex justify-between items-center mb-4">
              <h3 className="text-base font-bold text-white flex items-center gap-2"><Folder size={18} className="text-[#5842ff]" /> Available Cloud Profiles</h3>
              <button onClick={() => setShowCloudBrowser(false)} className="text-gray-500 hover:text-white p-1 bg-gray-800 rounded"><X size={16}/></button>
          </div>
          {cloudProfiles.length === 0 ? (
             <p className="text-sm text-gray-500 italic bg-[#0A0A0C] p-4 rounded-lg border border-gray-800 text-center">No profiles found in the cloud.</p>
          ) : (
             <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
               {cloudProfiles.map((prof, i) => (
                  <div key={i} className="bg-[#0A0A0C] border border-gray-700 p-4 rounded-lg flex flex-col justify-between hover:border-gray-500 transition">
                     <div className="mb-4">
                       <p className="text-sm font-black text-white mb-2">☁️ Profile #{i + 1}</p>
                       <p className="text-xs text-gray-400 flex items-center gap-1"><ListOrdered size={12} className="text-blue-400"/> {prof.rowsCount} Stream Rows</p>
                       <p className="text-xs text-gray-400 flex items-center gap-1 mt-1"><Link2 size={12} className="text-purple-400"/> {prof.urlsCount} Master URLs</p>
                     </div>
                     <div className="flex gap-2">
                        <button onClick={() => loadProfileFromBrowser(prof.actualKey)} className="flex-1 bg-green-500/10 text-green-400 hover:bg-green-500 hover:text-white py-2 rounded text-xs font-bold transition flex justify-center items-center gap-1"><CloudDownload size={14}/> Load File</button>
                        <button onClick={() => syncToCloud('delete', prof.actualKey)} className="px-3 bg-red-500/10 text-red-500 hover:bg-red-500 hover:text-white rounded transition" title="Delete this profile"><Trash2 size={14}/></button>
                     </div>
                  </div>
               ))}
             </div>
      ...
      <div className="mb-6 p-6 rounded-xl border border-gray-800 bg-[#12121A] relative overflow-hidden shadow-lg">
        ...
        <div className="flex flex-col md:flex-row gap-6">
          <div className="flex-1 flex flex-col items-center justify-center p-6 border-2 border-dashed border-[#5842ff]/40 rounded-xl bg-[#1A1A24]/50 hover:bg-[#1A1A24] transition">
            ...
            <p className="text-gray-400 text-sm text-center">Press <kbd className="bg-gray-800 px-2 py-1 rounded text-white font-mono">Ctrl + V</kbd></p>
          ...
          <div className="flex-1 flex flex-col gap-4 justify-center">
            <div className="flex gap-4">
              <div className="flex-1">
                <label className="flex items-center gap-2 text-sm font-semibold text-gray-400 mb-2">Custom AI Instructions</label>
                <input type="text" placeholder="e.g., Only extract semi-finals..." className="w-full bg-[#0A0A0C] border border-gray-700 rounded-lg p-3 text-white outline-none focus:border-[#5842ff] text-sm" value={customAIInstruction} onChange={(e) => setCustomAIInstruction(e.target.value)} />
              </div>
              <div className="w-1/4">
                <label className="flex items-center gap-2 text-sm font-semibold text-gray-400 mb-2">Start Row</label>
                <input type="number" min="1" className="w-full bg-[#0A0A0C] border border-gray-700 rounded-lg p-3 text-white outline-none focus:border-[#5842ff] text-sm text-center font-bold" value={aiInsertIndex} onChange={(e) => setAiInsertIndex(Number(e.target.value))} />
              </div>
            </div>
          ...
          <div className="mt-6 pt-6 border-t border-gray-800 flex flex-wrap gap-4">
            {pastedImages.map((imgSrc, idx) => (
              <div key={idx} className="relative w-24 h-24 rounded-lg overflow-hidden border-2 border-gray-700 group">
                <img src={imgSrc} alt="pasted" className="w-full h-full object-cover opacity-70" />
      ...
      <div className="bg-[#12121A] p-5 rounded-xl border border-[#5842ff]/30 mb-6 flex flex-col md:flex-row gap-6 items-start shadow-md">
        ...
        <div className="w-full md:w-2/3">
          <div className="flex justify-between items-center mb-3">
            ...
            <div className="flex gap-2">
              <button onClick={toggleSelectAllRows} className="bg-[#1A1A24] border border-gray-700 hover:bg-gray-800 text-gray-300 px-3 py-1.5 rounded-lg text-xs font-bold transition">Toggle All</button>
              <button onClick={applyMasterTime} className="bg-[#5842ff] hover:bg-[#4635d8] text-white px-4 py-1.5 rounded-lg text-xs font-bold transition shadow-lg">Apply to Selected</button>
            </div>
          </div>
            ...
            <div key={row.id} className={`bg-[#12121A] rounded-xl border ${borderClass} overflow-hidden transition-all shadow-md`}>
              <div className={`flex items-center justify-between p-4 ${bgClass} cursor-pointer`} onClick={() => updateRow(row.id, 'isExpanded', !row.isExpanded)}>
                <div className="flex items-center gap-4 flex-wrap">
                  ...
                  {!row.isExpanded && (
                    <div className="text-sm text-gray-400 flex gap-4 ml-4 items-center">
                      <span className="text-white font-medium">{row.channelId || 'No Channel'}</span>
                      <span className="truncate w-48 block text-xs text-gray-500">{row.targetUrls || 'No Links Selected'}</span>
              ...
              {row.isExpanded && (
                <div className="p-5 space-y-5">
                  <div className="bg-[#0A0A0C] p-4 rounded-lg border border-gray-800 shadow-inner">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                      ...
                      <div className="border-r border-gray-800/50 pr-5">
                        ...
                        {masterUrls.length === 0 ? (
                          <p className="text-xs text-gray-700 italic">Add master URLs globally first.</p>
                        ) : (
                          <div className="flex flex-wrap gap-3">
                  ...
                  {/* METADATA FORM CONTROL FIELD matrix */}
